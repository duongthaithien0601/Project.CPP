...
void dangKy(bool byManager = false) {
    ...
    // Danh dau la mat khau do he thong tu sinh
    if (byManager) {
        users[username].isAutoGenerated = true;
    }
...
bool dangNhap(string &loggedUser) {
    ...
    // doi mat khau neu mat khau tu sinh
    if (users[username].isAutoGenerated) {
        cout << "Ban dang nhap lan dau voi mat khau tu sinh. Vui long doi mat khau moi: ";
        string newPwd; cin >> newPwd;

        users[username].passwordHash   = hashPassword(newPwd);
        users[username].isAutoGenerated = false;
        luuDuLieu();
...
void capNhatThongTin(string &username) {
    ...
    cout << "Dang gui OTP xac thuc cap nhat thong tin toi email hien tai "
         << it->second.email << "...\n";
    if (!sendOTP(it->second.email, Otp1)) {
    ...
    // Neu doi gmail, gui OTP thang den gmail moi
    if (!newEmail.empty()) {
        string otp2 = sinhOTP();
        cout << "Dang gui OTP den email moi " << newEmail << "...\n";
        if (!sendOTP(newEmail, otp2)) {
    ...
    if (!newName.empty())     it->second.fullName = newName;
    if (!newEmail.empty())    it->second.email    = newEmail;
...
void capNhatThongTinBoiQuanLy(const string& manager, const string& targetUser) {
    ...
    auto mit = users.find(manager);
    if (mit == users.end() || !mit->second.isManager) {
    ...
    // Neu thay doi gmail, gui va xac thuc OTP den GMAIL MOI
    if (!newEmail.empty()) {
        string otp2 = sinhOTP();
        cout << "Dang gui OTP den email moi " << newEmail << "...\n";
        if (!sendOTP(newEmail, otp2)) {
